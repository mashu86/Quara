...
<div class="container-fluid px-2 px-md-4 py-3">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-2 mb-3 mb-md-4">
        <div>
            <h4 class="fw-bold mb-1"><i class="fa-solid fa-circle-plus text-success me-2"></i>Add New Income</h4>
        ...
        <div class="card-body p-3 p-md-5 income-card-body">
            <form action="{{ route('admin.incomes.store') }}" method="POST" onsubmit="return handleAdminFormSubmit(this);">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select name="status" id="status" class="form-select form-select-lg rounded-3 @error('status') is-invalid @enderror" required>
                            <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Active (Included in P&L Statement)</option>
                            <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>Inactive (Excluded from P&L)</option>
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" id="notes" class="form-control rounded-3 @error('notes') is-invalid @enderror" rows="3" placeholder="Enter customer details, order reference, or notes (optional)...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex flex-column flex-sm-row justify-content-end gap-2 border-top pt-3.5 mt-3">
                    ...
                    <button type="submit" class="btn btn-success rounded-pill px-4 fw-bold text-white shadow-sm order-1 order-sm-2">
