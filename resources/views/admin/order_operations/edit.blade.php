...
@section('title', 'Edit Order Operation #' . $operation->id . ' - ' . $siteName . ' Admin')
...
<div class="d-flex justify-content-between align-items-center mb-3 mb-md-4">
    <div>
        <h3 class="fw-bold mb-1 op-form-title">Edit Order Operation #{{ $operation->id }}</h3>
        <p class="text-muted small mb-0 op-form-subtitle">Update return/refund details or toggle ACTIVE / INACTIVE status for Order #{{ $operation->order->order_number }}</p>
    </div>
        ...
        <div class="col-lg-8">
                ...
                <div class="card-body p-4">
                    <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between gap-3">
                        ...
                        <div class="btn-group op-status-btn-group w-100 w-sm-auto" role="group" aria-label="Status Switch">
                            <input type="radio" class="btn-check" name="status" id="statusActive" value="active" {{ $operation->status === 'active' ? 'checked' : '' }}>
                            <label class="btn btn-outline-success fw-bold px-3 py-2 text-nowrap" for="statusActive">
                                <i class="fa-solid fa-circle-check me-1"></i> ACTIVE
                            </label>

                            <input type="radio" class="btn-check" name="status" id="statusInactive" value="inactive" {{ $operation->status === 'inactive' ? 'checked' : '' }}>
            ...
            <div class="card border-0 rounded-4 shadow-sm mb-3 mb-md-4">
                <div class="card-body p-4">
                        ...
                        @foreach($operationTypes as $key => $label)
                            <div class="col-6 col-sm-4">
                                <div class="form-check p-2 border rounded-3 bg-light hover-shadow op-type-card d-flex align-items-center">
                                    <input class="form-check-input ms-0 me-2 flex-shrink-0" type="radio" name="operation_type" id="type_{{ $key }}" value="{{ $key }}" {{ $operation->operation_type === $key ? 'checked' : '' }} onchange="toggleOtherDescription()">
                    ...
                    <div id="otherDescriptionBox" class="mb-2 {{ $operation->operation_type === 'other' ? '' : 'd-none' }}">
                        <label class="form-label fw-bold small">Specify Other Reason <span class="text-danger">*</span></label>
                        <input type="text" name="other_description" id="otherDescriptionInput" class="form-control rounded-3" placeholder="Enter custom operation details..." value="{{ old('other_description', $operation->other_description) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 rounded-4 shadow-sm mb-3 mb-md-4">
                <div class="card-body p-4">
                    <h5 class="fw-bold mb-3 border-bottom pb-2 text-dark">
                        <i class="fa-solid fa-box text-warning me-2"></i> Affected Item & Quantity
                    </h5>

                    <div class="row g-3">
                        <div class="col-sm-8">
                            <label class="form-label fw-bold small">Item Details</label>
                            <input type="text" class="form-control rounded-3 bg-light" readonly value="{{ $operation->orderItem ? $operation->orderItem->product_name . ' (Size: ' . $operation->orderItem->size . ')' : 'N/A' }}">
                        </div>
                        <div class="col-sm-4">
                            <label class="form-label fw-bold small">Affected Quantity (pcs) <span class="text-danger">*</span></label>
                            <input type="number" name="quantity" id="quantityInput" class="form-control rounded-3" value="{{ old('quantity', $operation->quantity) }}" min="1" required>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-flex flex-column flex-sm-row gap-2.5 gap-sm-4 align-items-start align-items-sm-center">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="is_product_restored" id="restoreNo" value="0" {{ !$operation->is_product_restored ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="is_product_restored" id="restoreYes" value="1" {{ $operation->is_product_restored ? 'checked' : '' }}>
            ...
            <div class="card border-0 rounded-4 shadow-sm mb-3 mb-md-4">
                <div class="card-body p-4">
                    ...
                    <div class="d-flex flex-column flex-sm-row gap-2.5 gap-sm-4 align-items-start align-items-sm-center mb-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="is_money_refunded" id="refundNo" value="0" {{ !$operation->is_money_refunded ? 'checked' : '' }} onchange="toggleRefundBox()">
                            <label class="form-check-label fw-bold text-dark small" for="refundNo">
                                No Refund Issued
                            </label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="is_money_refunded" id="refundYes" value="1" {{ $operation->is_money_refunded ? 'checked' : '' }} onchange="toggleRefundBox()">
                    ...
                    <div id="refundDetailsBox" class="p-3 bg-light rounded-3 border {{ $operation->is_money_refunded ? '' : 'd-none' }}">
                        <div class="row g-2.5 g-sm-3">
                            <div class="col-6 col-sm-4">
                                <label class="form-label fw-bold small">Product Refund (₹)</label>
                                <input type="number" step="0.01" name="product_refund_amount" id="productRefundInput" class="form-control rounded-3" value="{{ old('product_refund_amount', number_format($operation->product_refund_amount, 2, '.', '')) }}" min="0" oninput="calculateTotals()">
                            </div>
                            <div class="col-6 col-sm-4">
                                <label class="form-label fw-bold small">Delivery Refund (₹)</label>
                                <input type="number" step="0.01" name="delivery_refund_amount" id="deliveryRefundInput" class="form-control rounded-3" value="{{ old('delivery_refund_amount', number_format($operation->delivery_refund_amount, 2, '.', '')) }}" min="0" oninput="calculateTotals()">
                            </div>
                            <div class="col-12 col-sm-4">
                                <label class="form-label fw-bold small">Other Refund (₹)</label>
                                <input type="number" step="0.01" name="other_refund_amount" id="otherRefundInput" class="form-control rounded-3" value="{{ old('other_refund_amount', number_format($operation->other_refund_amount, 2, '.', '')) }}" min="0" oninput="calculateTotals()">
                            </div>
                            <div class="col-12 text-end border-top pt-2 mt-2">
                                <span class="fw-bold small text-dark">Total Refund Amount: </span>
                                <span class="fw-bold fs-5 text-danger" id="totalRefundDisplay">₹{{ number_format($operation->total_refund_amount, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div id="expenseRowsContainer">
                        @foreach($operation->expenses as $exp)
                            <div class="row g-2 mb-2 align-items-center" id="expenseRow_{{ $loop->index + 1 }}">
                                <div class="col-7">
                                    <input type="text" name="expenses[{{ $loop->index + 1 }}][description]" class="form-control form-control-sm rounded-2" placeholder="e.g. Repair charges / Courier return fee" value="{{ $exp->description }}" oninput="calculateTotals()">
                                </div>
                                <div class="col-4">
                                    <input type="number" step="0.01" name="expenses[{{ $loop->index + 1 }}][amount]" class="form-control form-control-sm rounded-2 expense-amount-input" placeholder="0.00" min="0" value="{{ number_format($exp->amount, 2, '.', '') }}" oninput="calculateTotals()">
                                </div>
                                <div class="col-1 text-end">
                                    <button type="button" class="btn btn-sm btn-outline-danger border-0 p-1" onclick="removeExpenseRow('expenseRow_{{ $loop->index + 1 }}')" title="Remove">
                    ...
                    <div id="noExpenseNotice" class="text-muted small {{ $operation->expenses->count() > 0 ? 'd-none' : '' }}">No extra operational expenses added yet.</div>
                </div>
            </div>
            ...
            <div class="card border-0 rounded-4 shadow-sm mb-3 mb-md-4">
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control rounded-3" rows="3" placeholder="Enter any specific notes regarding this return or test operation...">{{ old('notes', $operation->notes) }}</textarea>
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-warning rounded-pill fw-bold flex-grow-1 py-3 shadow-sm text-dark op-save-btn" style="background-color: var(--qw-gold); border-color: var(--qw-gold);">
                            <i class="fa-solid fa-floppy-disk me-1"></i> UPDATE OPERATION RECORD
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 rounded-4 shadow-sm mb-3 mb-md-4 border-top border-4 border-warning">
                <div class="card-body p-3.5 p-sm-4">
                    ...
                    <div class="d-flex justify-content-between mb-2 small">
                        ...
                        <span class="fw-bold text-danger" id="summaryTotalRefund">₹{{ number_format($operation->total_refund_amount, 2) }}</span>
                    </div>
                    <div class="d-flex justify-content-between mb-2 small">
                        <span class="text-muted">Additional Expenses:</span>
                        <span class="fw-bold text-danger" id="summaryTotalExpense">₹{{ number_format($operation->additional_expense_total, 2) }}</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <span class="fs-4 fw-bold text-danger" id="summaryTotalAdjustment">-₹{{ number_format($operation->total_financial_adjustment, 2) }}</span>
                    </div>
...
@section('scripts')
<script>
    let expenseRowIndex = {{ $operation->expenses->count() }};
