...
<div class="card border-0 rounded-4 shadow-sm mb-4">
    <div class="card-body p-0">
        <div class="table-responsive" id="incomes-scroll-container" style="max-height: 75vh; overflow-y: auto;">
            <table class="table align-middle mb-0">
                ...
                <tbody id="incomes-tbody">
                    @forelse($incomes as $inc)
                        @include('admin.incomes.partials.desktop_rows', ['incomes' => collect([$inc])])
                    @empty
                        <tr>
    ...
    <div class="card-footer bg-white py-2 text-center border-top">
        <div id="infinite-scroll-loading" class="d-none text-muted small py-1">
            ...
            Loading more income records...
        </div>
        <div id="infinite-scroll-end" class="{{ $incomes->hasMorePages() ? 'd-none' : '' }} text-muted small py-1">
            <i class="fa-solid fa-circle-check text-success me-1"></i> All {{ $incomes->total() }} income records loaded
        </div>
    </div>
</div>
...
function showIncomeDetail(incomeId) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        let nextPageUrl = @json($incomes->nextPageUrl());
        let hasMore = @json($incomes->hasMorePages());
        let isLoading = false;

        const scrollContainer = document.getElementById('incomes-scroll-container');
        ...
        function fetchNextPage() {
            ...
            .catch(err => {
                console.error('Error fetching more income records:', err);
                isLoading = false;
                if (loadingSpinner) loadingSpinner.classList.add('d-none');
            });
        }
