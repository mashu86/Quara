...
@section('title', 'Edit Shipping Policy - QUARA WALDROP Admin')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3 mb-md-4 gap-2">
    <div>
        <h4 class="fw-bold mb-0 text-truncate" style="font-size: 0.95rem; max-width: 65%;">Edit Shipping Policy</h4>
        <p class="text-muted small mb-0 d-none d-sm-block">Modifying policy: <strong>{{ $shippingPolicy->name }}</strong></p>
    </div>
...
<form action="{{ route('admin.shipping-policies.update', $shippingPolicy->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="card border-0 rounded-4 shadow-sm max-w-800 mx-auto overflow-hidden">
        <div class="card-header bg-dark text-white p-3 p-sm-4">
            <h5 class="fw-bold mb-0 text-warning" style="font-size: 0.88rem;"><i class="fa-solid fa-pen-to-square me-2"></i> Edit Policy Configuration</h5>
        </div>
        <div class="card-body p-3 p-sm-5">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" class="form-control rounded-3 py-2" style="font-size: 0.85rem;" value="{{ old('name', $shippingPolicy->name) }}" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="criteria_type" id="criteriaTypeSelect" class="form-select rounded-3 py-2 fw-bold" style="font-size: 0.85rem;" required onchange="updateCriteriaLabels()">
                    <option value="cart_price" {{ old('criteria_type', $shippingPolicy->criteria_type) === 'cart_price' ? 'selected' : '' }}>💰 Cart Price Subtotal (₹)</option>
                    <option value="cart_count" {{ old('criteria_type', $shippingPolicy->criteria_type) === 'cart_count' ? 'selected' : '' }}>📦 Cart Item Count (Total Quantity)</option>
                </select>
            </div>
                    ...
                    <div class="p-3 bg-light border rounded-4 h-100">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="from_operator" class="form-select form-select-sm rounded-3" required>
                                <option value=">=" {{ old('from_operator', $shippingPolicy->from_operator) === '>=' ? 'selected' : '' }}>&ge; Greater than or Equal to</option>
                                <option value=">" {{ old('from_operator', $shippingPolicy->from_operator) === '>' ? 'selected' : '' }}>&gt; Greater than</option>
                                <option value="<=" {{ old('from_operator', $shippingPolicy->from_operator) === '<=' ? 'selected' : '' }}>&le; Less than or Equal to</option>
                                <option value="<" {{ old('from_operator', $shippingPolicy->from_operator) === '<' ? 'selected' : '' }}>&lt; Less than</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="p-3 bg-light border rounded-4 h-100">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="to_operator" class="form-select form-select-sm rounded-3">
                                ...
                                <option value="<=" {{ old('to_operator', $shippingPolicy->to_operator) === '<=' ? 'selected' : '' }}>&le; Less than or Equal to</option>
                                <option value="<" {{ old('to_operator', $shippingPolicy->to_operator) === '<' ? 'selected' : '' }}>&lt; Less than</option>
                                <option value=">=" {{ old('to_operator', $shippingPolicy->to_operator) === '>=' ? 'selected' : '' }}>&ge; Greater than or Equal to</option>
                                <option value=">" {{ old('to_operator', $shippingPolicy->to_operator) === '>' ? 'selected' : '' }}>&gt; Greater than</option>
                            </select>
                        </div>
                        <div>
            ...
            <div class="mb-4">
                ...
                <div class="row g-3">
                    <div class="col-sm-6">
                        <input type="radio" class="btn-check" name="delivery_type" id="deliveryFree" value="free" {{ old('delivery_type', $shippingPolicy->delivery_type) === 'free' ? 'checked' : '' }} onchange="toggleCustomChargeBox()" autocomplete="off">
                    ...
                    <div class="col-sm-6">
                        <input type="radio" class="btn-check" name="delivery_type" id="deliveryCustom" value="custom" {{ old('delivery_type', $shippingPolicy->delivery_type) === 'custom' ? 'checked' : '' }} onchange="toggleCustomChargeBox()" autocomplete="off">
            ...
            <div id="customChargeBox" class="mb-4 d-none p-4 bg-warning-subtle border border-warning rounded-4">
                ...
                <div class="input-group">
                    ...
                    <input type="number" step="0.01" name="charge_amount" id="chargeAmountInput" class="form-control form-control-lg rounded-end-3 fw-bold" value="{{ old('charge_amount', $shippingPolicy->charge_amount) }}" min="0">
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="number" name="priority" class="form-control rounded-3" value="{{ old('priority', $shippingPolicy->priority) }}" min="0">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-bold" style="font-size: 0.82rem;">Policy Status <span class="text-danger">*</span></label>
                    <select name="status" class="form-select rounded-3" required>
                        <option value="active" {{ old('status', $shippingPolicy->status) === 'active' ? 'selected' : '' }}>🟢 Active (Enable Rule)</option>
                        <option value="inactive" {{ old('status', $shippingPolicy->status) === 'inactive' ? 'selected' : '' }}>🔴 Inactive (Disable Rule)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="col-12 mt-3 mt-md-4 text-center text-sm-start">
                <button type="submit" class="btn btn-warning rounded-pill fw-bold w-100 w-sm-auto px-4 px-sm-5 py-2.5 py-sm-2 shadow-sm text-dark" style="font-size: 0.82rem; background-color: var(--qw-gold); border-color: var(--qw-gold);">
                    <i class="fa-solid fa-floppy-disk me-1"></i> UPDATE POLICY
                </button>
            </div>
        </div>
    </div>
